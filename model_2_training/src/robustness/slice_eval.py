...
def _get_dataset(pred: dict) -> str:
    ...
    Checks metadata.source first, then metadata.dataset, then falls back
    to "unknown". Returns a string safe for use as a dict key.
    """
    sample = pred.get("sample") or {}
    metadata = sample.get("metadata") or {}
    source = metadata.get("source") or metadata.get("dataset") or "unknown"
    return str(source)
...
def compute_all_slices(predictions: list[dict]) -> dict[str, dict[str, dict]]:
    """
    Run slice evaluation across all four slice dimensions.
