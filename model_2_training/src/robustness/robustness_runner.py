...
----------------
  ...
  slice_report.json            Per-slice metrics across all 5 dimensions
  cross_slice_robustness.json  Per-slice robustness deltas (archetype × perturbation)
  paraphrase_consistency.json  Agreement rate across 3 paraphrase seeds
...
from model_2_training.src.tracking.wandb_logger import (
    init_run, log_eval_metrics, log_metrics, finish_run,
)


# ---------------------------------------------------------------------------
# Cross-slice perturbation analysis (Gap 3)
# ---------------------------------------------------------------------------

def _run_cross_slice_perturbations(
    samples: list[dict],
    model,
    tokenizer,
    gen_kwargs: dict,
    perturbation_types: list[str],
    slice_dim: str = "archetype",
    seed: int = 42,
) -> dict:
    """
    For each slice value in slice_dim, apply each perturbation and compute delta.

    Answers questions like: "does paraphrase hurt multi_step more than single_step?"

    Args:
        samples:            Clean samples.
        model / tokenizer:  Already-loaded model (reused, no reload).
        gen_kwargs:         Generation kwargs forwarded to run_generation().
        perturbation_types: Perturbation types to test within each slice.
        slice_dim:          Slice dimension to group by (default: "archetype").
        seed:               Base random seed.

    Returns:
        {slice_value: build_robustness_summary output} for each slice group.
    """
    extractor = _SLICE_EXTRACTORS[slice_dim]
    groups: dict[str, list[dict]] = {}
    for s in samples:
        key = extractor({"sample": s, "generated": "", "reference": ""})
        groups.setdefault(key, []).append(s)

    results = {}
    for slice_val, group_samples in groups.items():
        logger.info(f"Cross-slice: {slice_dim}={slice_val!r}  n={len(group_samples)}")
        clean_preds = run_generation(
            samples=group_samples, model=model, tokenizer=tokenizer, **gen_kwargs
        )
        clean_metrics = run_all_metrics(clean_preds)
        pert_metrics_map = {}
        for pert_type in perturbation_types:
            pert_samples = perturb_dataset(group_samples, pert_type, seed=seed)
            pert_preds = run_generation(
                samples=pert_samples, model=model, tokenizer=tokenizer, **gen_kwargs
            )
            pert_metrics_map[pert_type] = run_all_metrics(pert_preds)
        results[slice_val] = build_robustness_summary(clean_metrics, pert_metrics_map)
    return results


# ---------------------------------------------------------------------------
# Paraphrase consistency metric (Gap 4)
# ---------------------------------------------------------------------------

def _compute_paraphrase_consistency(
    samples: list[dict],
    model,
    tokenizer,
    gen_kwargs: dict,
    ...
    """
    Run n_seeds different paraphrase variants on the same samples.

    Agreement rate = fraction of samples where all seeds produce the same
    error_taxonomy category. A high agreement rate means the model's output
    category is stable across sentence reorderings even if quality varies.

    Args:
        samples:   Clean samples.
        n_seeds:   Number of independent paraphrase seeds to compare.
        base_seed: First seed; subsequent seeds are base_seed + i*100.

    Returns:
        Dict with agreement_rate, n_seeds, n_samples, and per-seed category counts.
    """
    per_seed_categories: list[list[str]] = []
    for i in range(n_seeds):
        pert_samples = perturb_dataset(samples, "paraphrase", seed=base_seed + i * 100)
        preds = run_generation(
            samples=pert_samples, model=model, tokenizer=tokenizer, **gen_kwargs
        )
        categories = [
            _classify_output(p.get("generated", ""), p.get("reference", ""))
            for p in preds
        ]
        per_seed_categories.append(categories)

    n = len(samples)
    agreed = sum(
        len({per_seed_categories[s][i] for s in range(n_seeds)}) == 1
        for i in range(n)
    )
    return {
        "n_seeds": n_seeds,
        ...
        "per_seed_category_counts": [
            {cat: cats.count(cat) for cat in set(cats)}
            for cats in per_seed_categories
        ],
    }
...
def run_robustness_eval(
    ...
    init_run(
        ...
        run_name=f"phase3-robustness-{checkpoint_path.name}",
        tags=["phase3", "robustness"],
    )
    ...
    log_eval_metrics(clean_metrics, split="robustness/clean")  # Gap 5
    ...
    # Gap 5 — log perturbed metrics to W&B
    for pert_type, m in perturbed_metrics.items():
        log_eval_metrics(m, split=f"robustness/{pert_type}")
    ...
    log_metrics({
        "robustness/worst_perturbation_delta": robustness_summary.get(
            "worst_perturbation_total_delta", 0
        )
    })  # Gap 5
    ...
    # 4b. Cross-slice perturbation analysis (Gap 3)
    logger.info("=" * 60)
    logger.info("Phase 3 — Cross-slice perturbation analysis")
    logger.info("=" * 60)
    cross_slice = _run_cross_slice_perturbations(
        samples=samples, model=model, tokenizer=tokenizer,
        gen_kwargs=gen_kwargs, perturbation_types=perturbation_types,
        slice_dim="archetype", seed=seed,
    )
    _save_json(cross_slice, output_dir / "cross_slice_robustness.json")

    # 4c. Paraphrase consistency (Gap 4)
    logger.info("=" * 60)
    logger.info("Phase 3 — Paraphrase consistency")
    logger.info("=" * 60)
    consistency = _compute_paraphrase_consistency(
        samples=samples, model=model, tokenizer=tokenizer, gen_kwargs=gen_kwargs,
    )
    _save_json(consistency, output_dir / "paraphrase_consistency.json")
    logger.info(f"  Paraphrase agreement rate: {consistency['agreement_rate']:.1%}")
