"""
Takes one raw dict -> returns (training_record | None, status_str).
No file reading, no writing, no config loading happens here.
This file only contains pure functions that transform data.

Steps applied to every row:
  1. Filter      -> reject rows that don't meet our quality criteria
  2. Redact PII  -> remove IPs, emails, API keys before training
  3. Re-validate -> make sure redaction didn't break the YAML structure
  4. Synthesize  -> turn the filename into a natural-language prompt
  5. Wrap        -> package everything into the Track B workflow JSON format
"""

import os
import re
import json
import yaml
from typing import Optional

# Temporary local system prompt for DS6.
# Later, move this into one shared Track B prompt module and import it here.
SYSTEM_PROMPT = (
    'You are AutoMend. Return only strict JSON. '
    'Available tools: '
    '[{"tool":"apply_manifest","params":{"manifest_content":"string"}}]'
)

# setup functions
def build_redactors(cfg: dict) -> list[tuple[re.Pattern, str]]:
    pats = cfg["redaction"]["patterns"]
    repls = cfg["redaction"]["replacements"]
    return [
        (re.compile(pats["ipv4"]), repls["ipv4"]),
        (re.compile(pats["api_key"]), repls["api_key"]),
        (re.compile(pats["email"]), repls["email"]),
        (re.compile(pats["creds"], re.IGNORECASE), repls["creds"]),
    ]


def build_prompt_rules(cfg: dict) -> list[dict]:
    return cfg["prompt_rules"]


# helper functions
def _collect_licenses(row: dict) -> list[str]:
    """
    Collect license strings from the three Stack license fields.
    Return lowercased values so allowlist matching stays consistent.
    """
    out: list[str] = []
    for key in (
        "max_stars_repo_licenses",
        "max_issues_repo_licenses",
        "max_forks_repo_licenses",
    ):
        v = row.get(key) or []
        if isinstance(v, list):
            out.extend(x for x in v if isinstance(x, str) and x.strip())
    return [x.strip().lower() for x in out]


def _best_path(row: dict) -> str:
    """Pick the first non-empty repo path (stars -> issues -> forks)."""
    for key in ("max_stars_repo_path", "max_issues_repo_path", "max_forks_repo_path"):
        v = row.get(key)
        if isinstance(v, str) and v.strip():
            return v.strip()
    return ""


def _k8s_ok(content: str, f: dict) -> bool:
    """
    Return True only if content passes all enabled K8s detection checks.
    If all three flags are off, return True.
    """
    if f.get("require_api_version") and "apiVersion" not in content:
        return False
    if f.get("require_kind") and "kind:" not in content:
        return False
    if f.get("require_metadata") and "metadata:" not in content:
        return False
    return True


def _has_ml_keyword(content: str, cfg: dict) -> bool:
    """Return True if content contains any configured ML infra keyword."""
    cl = content.lower()
    for group in (cfg["filters"].get("ml_infra_groups") or []):
        for kw in (cfg["keywords"].get(group) or []):
            if kw.lower() in cl:
                return True
    return False


# filter
def passes_filter(row: dict, cfg: dict) -> tuple[bool, str]:
    """
    Decide whether to keep or reject a row.
    Return (True, "ok") if the row passes all checks.
    Return (False, reason) as soon as one check fails.
    """
    f = cfg["filters"]
    content = row.get("content") or ""
    size = int(row.get("size") or 0)
    af = float(row.get("alphanum_fraction") or 0.0)
    ext = (row.get("ext") or "").lstrip(".").lower()
    licenses = _collect_licenses(row)

    if not content:
        return False, "empty_content"
    if size < f["min_size_bytes"]:
        return False, "too_small"
    if size > f["max_size_bytes"]:
        return False, "too_large"
    if af < f["min_alphanum_frac"]:
        return False, "low_alphanum"

    # file extension check
    allowed = [e.lstrip(".").lower() for e in (f.get("allowed_exts") or [])]
    if allowed and ext and ext not in allowed:
        return False, "bad_extension"

    # Kubernetes-manifest checks
    if f.get("require_api_version") and "apiVersion" not in content:
        return False, "not_k8s"
    if f.get("require_kind") and "kind:" not in content:
        return False, "missing_kind"
    if f.get("require_metadata") and "metadata:" not in content:
        return False, "missing_metadata"

    # YAML parse check before keyword checks
    if f.get("valid_yaml_only"):
        try:
            yaml.safe_load(content)
        except yaml.YAMLError:
            return False, "invalid_yaml"

    # license check
    allowlist = [l.lower() for l in (f.get("permissive_licenses") or [])]
    if allowlist:
        if f.get("require_license") and not licenses:
            return False, "missing_license"
        if licenses and not any(l in allowlist for l in licenses):
            return False, "bad_license"

    # ML infra check
    if f.get("require_ml_infra"):
        mode = (f.get("ml_infra_mode") or "strict").lower()
        has_kw = _has_ml_keyword(content, cfg)
        has_k8s = _k8s_ok(content, f)

        if mode == "strict":
            if not (has_k8s and has_kw):
                return False, "not_ml_infra"
        else:
            if not (has_k8s or has_kw):
                return False, "not_ml_infra"

    return True, "ok"


# PII redaction
def redact(content: str, redactors: list[tuple[re.Pattern, str]]) -> str:
    """Replace all PII patterns with placeholder tokens."""
    for pattern, repl in redactors:
        content = pattern.sub(repl, content)
    return content


# prompt synthesis
def synthesize_prompt(filepath: str, rules: list[dict]) -> str:
    """Turn a filename into a natural-language instruction."""
    name = os.path.basename(filepath or "manifest.yaml")
    slug = re.sub(r"\.(yaml|yml)$", "", name, flags=re.I)
    slug = re.sub(r"[-_]+", " ", slug).strip()

    for rule in rules:
        if rule["match"] == "*" or rule["match"] in slug.lower():
            return rule["template"].format(slug=slug)

    if slug:
        return f"Apply the following Kubernetes manifest for {slug}"
    return "Apply the following Kubernetes manifest"


def wrap(content: str) -> str:
    """
    Package the YAML into the shared Track B workflow JSON format.
    json.dumps handles escaping for newlines, quotes, and backslashes.
    """
    return json.dumps(
        {
            "workflow": {
                "steps": [
                    {
                        "step_id": 1,
                        "tool": "apply_manifest",
                        "params": {
                            "manifest_content": content
                        },
                    }
                ]
            }
        },
        ensure_ascii=False,
    )


def process_row(
    row: dict,
    cfg: dict,
    redactors: list[tuple[re.Pattern, str]],
    prompt_rules: list[dict],
) -> tuple[Optional[dict], str]:
    """
    Return (training_record, "ok") on success.
    Return (None, reason_str) on rejection.
    """
    ok, reason = passes_filter(row, cfg)
    if not ok:
        return None, reason

    content = redact(row["content"], redactors)

    # re-validate after redaction
    try:
        yaml.safe_load(content)
    except yaml.YAMLError:
        return None, "invalid_yaml_post_redaction"

    filepath = _best_path(row)
    prompt = synthesize_prompt(filepath, prompt_rules)
    wrapped = wrap(content)

    return {
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": prompt},
            {"role": "assistant", "content": wrapped},
        ],
        "_meta": {
            "hexsha": row.get("hexsha"),
            "path": filepath,
            "size": row.get("size"),
            "licenses": _collect_licenses(row),
        },
    }, "ok"