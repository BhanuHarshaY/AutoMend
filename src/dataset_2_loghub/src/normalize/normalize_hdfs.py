"""Normalize HDFS_2k.log_structured.csv into the unified schema.

Actual columns: LineId, Date, Time, Pid, Level, Component, Content, EventId, EventTemplate
Level is standard (INFO, WARN, ERROR, FATAL, DEBUG) — direct mapping.
Timestamp: numeric strings "081109 203615" (YYMMDD HHMMSS) — kept as string.
"""
import json
import sys
...
INPUT  = RAW_DIR / "HDFS" / "HDFS_2k.log_structured.csv"
OUTPUT = PROCESSED_DIR / "normalized" / "hdfs.parquet"
...
ERROR_KW = ["exception", "failed", "failure", "error", "unable", "corrupt", "denied"]
...
def normalize_hdfs(input_path: Path = INPUT, output_path: Path = OUTPUT):
